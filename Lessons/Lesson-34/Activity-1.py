...
class Sprite(pygame.sprite.Sprite):
  ...
  def move(self, x_change, y_change):
    self.rect.x = max(min(self.x + x_change, SCREEN_WIDTH - self.rect.width),
                      0)
    self.rect.y = max(min(self.y + y_change, SCREEN_WIDTH - self.rect.width),
                      0)
...
while running:
  ...
  if not won:
    ...
    if sprite1.rect.colliderect(sprite1.rect):
